...
function processBankStatement(rawData) {
  ...
  function parseDate(dateString) {
    ...
    const parsers = [
      ...
      (str) => { // DD/MM/YY  (handle 2-digit year)
        const [day, month, year] = str.split('/').map(Number);
        ...
        if (value) {
          ...
          if (typeof value === 'string') {
            ...
            if (lowerValue.includes("bank name")) {
              bank_details.bank_name = nextValue || null;
            } else if (lowerValue.includes("account holder name") || lowerValue.includes("account name")) {
              bank_details.account_holder_name = nextValue || null;
              ...
              bank_details.ifsc = nextValue || null;
            } else if (lowerValue.includes("branch name")) {
              bank_details.branch_name = nextValue || null;
            } else if (lowerValue.includes("branch code")) {
                bank_details.branch_code = nextValue || null;
              ...
              const balValue = typeof nextValue === 'number' ? nextValue : parseFloat((nextValue + '').replace(/[^0-9.-]+/g, ''));
             ...
             for (const key in row) {
                if (row.hasOwnProperty(key)) {
                     ...
                     if(value && typeof value === 'string'){
                        const lowerValue = value.toLowerCase();
                         if (lowerValue.includes("rtgs/neft ifsc")) {
                                  const ifscValue = lowerValue.split(':');
                                  if(ifscValue.length>1){
                                     bank_details.ifsc = ifscValue[1].trim().split(' ')[0];
                                  }
                        }
                     }
                  }
